...
def predict_many(data, model, hypos, hypers, reverse, attention):
    ...
    for hypon, hyper in zip(hypos, hypers):
        ...
        # out of pattern mode
            ...
            try:
                ...
                hypon_word_context = data.context_dict[hypon_id]
                hyper_word_context = data.context_dict[hyper_id]
                inputs = torch.tensor(np.asarray([[hypon_word_context, hyper_word_context]]), dtype=torch.long)
              
                if reverse:
                    inputs = torch.tensor(np.asarray([[hyper_word_context, hypon_word_context]]), dtype=torch.long)
                    if attention:
                        pred = model(inputs)[0].detach().cpu().numpy()[0]
                    else:
...
def detection_setup(file_name, model, matrix_data ,device, attention):
    ...
    for x,y in zip(ds.hypos, ds.hypers):
        if x in matrix_data.vocab and y in matrix_data.vocab:
            ...
            try:
                ...
                hypon_word_context = matrix_data.context_dict[hypon_id]
                hyper_word_context = matrix_data.context_dict[hyper_id]
                inputs = torch.tensor(np.asarray([[hypon_word_context, hyper_word_context]]), dtype=torch.long)
                #if model.name  == "self":
                if attention:
                    score = model(inputs)[0].detach().cpu().numpy()[0]
                else:
                    score = model(inputs).detach().cpu().numpy()[0]
                count_context +=1
    ...
    logger.info("Context : AP for validation is :{} || for test is :{}".format(average_precision_score(y[m_val],h[m_val]), 
...
def hyperlex_setup(file_name, model, matrix_data,device, attention):
    ...
    logger.info("Context: train cor: {} | test cor:{}".format(result['spearman_train'],result['spearman_test']))
...
def dir_bless_setup(model, matrix_data ,device, attention):
    ...
    for hypon, hyper in zip(hypos, hypers):
        ...
        # out of pattern mode
            ...
            try:
                ...
                if attention:
                    forward_pred = model(inputs)[0].detach().cpu().numpy()[0]
                else:
                    forward_pred = model(inputs).detach().cpu().numpy()[0]
                
                inputs = torch.tensor(np.asarray([[hyper_word_context, hypon_word_context]]), dtype=torch.long)

                if attention:
                    reverse_pred = model(inputs)[0].detach().cpu().numpy()[0]
                else:
                    reverse_pred = model(inputs).detach().cpu().numpy()[0]
...
def dir_wbless_setup(model, data ,device,attention):
    ...
    h, h_svd, _ = predict_many(data, model, ds.hypos, ds.hypers, False, attention)
    y = ds.y
...
def dir_bibless_setup(model, data ,device, attention):
    ...
    hf, hf_svd, oop_rate = predict_many(data, model, ds.hypos, ds.hypers, False,attention)
    hr, hr_svd, _ = predict_many(data, model, ds.hypos, ds.hypers, True, attention)
...
def evaluation_all(config, ckpt_path):
    ...
    model = init_model(config, ckpt_path, matrix_data.context_word_emb, device)
    ...
    for taskname, filename in SIEGE_EVALUATIONS:
        result = detection_setup(filename, model, matrix_data ,device, attention)
        results["detec_{}".format(taskname)] = result

    for taskname, filename in CORRELATION_EVAL_DATASETS:
        result = hyperlex_setup(filename, model, matrix_data, device, attention)
        results["corr_{}".format(taskname)] = result

    dir_bless_setup(model,matrix_data, device, attention)
    dir_wbless_setup(model, matrix_data, device,attention)
    dir_bibless_setup(model, matrix_data, device,attention)
...
if __name__ == "__main__":
    ...
    log_path = os.path.join(ckpt_dir, 'context.log')
